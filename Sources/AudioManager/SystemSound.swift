...
extension SystemSound {
  ...
  public var url: URL? {
    ...
    case .modern(let modernSound):
      return URL(fileURLWithPath: modernSound.path)
    case .nano(let nanoSound):
      return URL(fileURLWithPath: nanoSound.path)
    case .new(let newSound):
      return URL(fileURLWithPath: newSound.path)
    case .ui(let uISound):
      return URL(fileURLWithPath: uISound.path)
    }
  }
}
